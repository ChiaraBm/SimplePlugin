...
public class PluginDiscoveryGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        {
            var pluginInterface = compilation.GetTypeByMetadataName("SimplePlugin.Abstractions.IPluginModule");
            var pluginAttribute = compilation.GetTypeByMetadataName("SimplePlugin.Abstractions.PluginModuleAttribute");
    ...
    private static string GenerateRegistration(List<string> modules)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("using SimplePlugin.Abstractions;");
        sb.AppendLine();
        sb.AppendLine("namespace SimplePlugin.Generated;");
        sb.AppendLine();
        sb.AppendLine("public static class PluginRegistry");
        sb.AppendLine("{");
        sb.AppendLine("    public static IPluginModule[] Modules");
        sb.AppendLine("    {");

        if (modules.Count == 0)
        {
            sb.AppendLine("        get => System.Array.Empty<IPluginModule>();");
        }
        else
        {
            sb.AppendLine("        get => new IPluginModule[]");
            sb.AppendLine("        {");
